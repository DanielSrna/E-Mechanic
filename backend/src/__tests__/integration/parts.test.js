...
beforeAll(async () => {
  ...
  await User.create({
    name: 'Admin', email: 'admin.part@test.dev', cedula: '7777777777',
...
describe('Parts API', () => {
  describe('POST /api/parts', () => {
    it('crea repuesto con SKU en mayúsculas', async () => {
      const res = await request(app)
        ...
        .send({
          sku: 'OIL-10W40', name: 'aceite sintetico',
          purchasePrice: 25000, salePrice: 45000, stock: 15,
        });
    ...
    it('rechaza SKU duplicado con 409', async () => {
      await request(app).post('/api/parts')
        .set('Authorization', `Bearer ${adminToken}`)
        .send({ sku: 'DUP-001', name: 'Uno', purchasePrice: 1000, salePrice: 2000 });

      const res = await request(app).post('/api/parts')
        .set('Authorization', `Bearer ${adminToken}`)
        .send({ sku: 'DUP-001', name: 'Dos', purchasePrice: 1000, salePrice: 2000 });
  ...
  describe('GET /api/parts', () => {
    beforeEach(async () => {
      await request(app).post('/api/parts')
        .set('Authorization', `Bearer ${adminToken}`)
        .send({ sku: 'SKU-A', name: 'Repuesto A', purchasePrice: 1000, salePrice: 2000, stock: 10, minStock: 5 });
      await request(app).post('/api/parts')
        .set('Authorization', `Bearer ${adminToken}`)
        .send({ sku: 'SKU-B', name: 'Repuesto B', purchasePrice: 1000, salePrice: 2000, stock: 2, minStock: 5 });
    });
  ...
  describe('PUT /api/parts/:id', () => {
    it('actualiza stock y precio', async () => {
      const created = await request(app).post('/api/parts')
        .set('Authorization', `Bearer ${adminToken}`)
        .send({ sku: 'UPD-001', name: 'Update Me', purchasePrice: 1000, salePrice: 2000, stock: 5 });
  ...
  describe('DELETE /api/parts/:id', () => {
    it('elimina repuesto', async () => {
      const created = await request(app).post('/api/parts')
        .set('Authorization', `Bearer ${adminToken}`)
        .send({ sku: 'DEL-001', name: 'Delete Me', purchasePrice: 1000, salePrice: 2000 });
