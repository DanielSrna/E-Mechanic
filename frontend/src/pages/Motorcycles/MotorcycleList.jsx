  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <Link to="/motorcycles/new" className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm font-medium text-center">
          + Registrar Moto
        </Link>
      </div>

      <input value={plate} onChange={e => { setPlate(e.target.value.toUpperCase()); fetch(e.target.value.toUpperCase()); }}
        placeholder="Buscar por placa..." className="w-full sm:w-64 px-4 py-2 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 outline-none text-sm" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {motos.map(m => (
          <div key={m._id} className="bg-white rounded-xl shadow-sm p-5 hover:shadow-md transition">
